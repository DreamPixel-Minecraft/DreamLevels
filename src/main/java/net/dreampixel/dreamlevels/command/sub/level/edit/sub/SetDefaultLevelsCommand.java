...
@CommandInfo(
        ...
        permissions = "DreamLevels.Commands.Edit.SetDefaultLevels"
)
public class SetDefaultLevelsCommand extends SubCommand {
    @Override
    public boolean execute(@NotNull CommandContext ctx) {
        var level = ctx.arguments()[2].get(Level.class);
        var value = ctx.arguments()[3].getInt();
        level.setDefaultLevels(value);
        LocaleUtils.sendCmdMessage(ctx.sender(), "modify.set-default-levels",
                "{level}", level.getName(),
