...
@CommandInfo(
        name = "SetMultiple",
        permissions = "DreamLevels.Commands.SetMultiple"
)
public class SetMultipleCommand extends LevelCommand {
    ...
    public boolean executeOnline(@NotNull CommandContext ctx, @NotNull Level level, double value) {
        ...
        players.forEach(p -> level.getLevelData(p).setMultiple(value));
        return true;
    }
    ...
    public void sendOnlineFeedback(@NotNull CommandContext ctx, @NotNull Level level, double value) {
        ...
        players.forEach(p -> {
            ...
            LocaleUtils.sendCmdMessage(ctx.sender(), "level.set-multiple",
                    "{player}", p.getName(),
                    "{level}", level.getName(),
