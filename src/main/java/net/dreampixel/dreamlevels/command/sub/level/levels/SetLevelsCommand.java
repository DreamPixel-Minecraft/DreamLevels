...
@CommandInfo(
        name = "SetLevels",
        permissions = "DreamLevels.Commands.SetLevels"
)
public class SetLevelsCommand extends LevelCommand {
    ...
    public boolean executeOnline(@NotNull CommandContext ctx, @NotNull Level level, double value) {
        ...
        players.forEach(p -> level.getLevelData(p).setLevels((int) value, true));
        return true;
    }
    ...
    public void sendOnlineFeedback(@NotNull CommandContext ctx, @NotNull Level level, double value) {
        var players = (PlayerSet) ctx.getProperty("players");
        LocaleUtils.sendCmdMessage(ctx.sender(), "level.set-levels",
